...
// Fetch current Trek record early for reference in POST validation/uploads
try {
    $trek_stmt = $db->prepare("SELECT * FROM treks WHERE id = ? LIMIT 1");
    $trek_stmt->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $altitude = (int)($trek['altitude'] ?? 0); // Keep existing value for backward compatibility
    
    // New Package configuration inputs
    ...
    if (empty($title) || empty($duration)) {
        ...
        try {
            // Check if slug is unique (excluding current trek)
            $slug_check = $db->prepare("SELECT id FROM treks WHERE slug = ? AND id != ? LIMIT 1");
            ...
            if (empty($error)) {
                ...
                // Step 29: Record slug change redirect to ensure no broken links
                $old_slug = $trek['slug'] ?? '';
                if (!empty($old_slug) && !empty($slug) && $old_slug !== $slug) {
                    $redir_check = $db->prepare("SELECT id FROM trek_slug_redirects WHERE old_slug = ? AND new_slug = ? LIMIT 1");
                    $redir_check->execute([$old_slug, $slug]);
                    if (!$redir_check->fetchColumn()) {
                        $db->prepare("INSERT INTO trek_slug_redirects (trek_id, old_slug, new_slug) VALUES (?, ?, ?)")
                           ->execute([$id, $old_slug, $slug]);
                    }
                }

                $set_clauses = [];
                $params = [];
                foreach ($trek_data as $col => $val) {
                    $set_clauses[] = "`{$col}` = ?";
                    $params[] = $val;
                }
                $params[] = $id;

                $stmt = $db->prepare("UPDATE treks SET " . implode(', ', $set_clauses) . " WHERE id = ?");
                $stmt->execute($params);

                // Delete existing dynamic records
                $db->prepare("DELETE FROM trek_highlights WHERE trek_id = ?")->execute([$id]);
                $db->prepare("DELETE FROM trek_notes WHERE trek_id = ?")->execute([$id]);
                $db->prepare("DELETE FROM trek_additional_notes WHERE trek_id = ?")->execute([$id]);
                $db->prepare("DELETE FROM trek_faqs WHERE trek_id = ?")->execute([$id]);
                ...
                for ($i = 0; $i < count($hl_texts); $i++) {
                    ...
                    if (!empty($hl_text)) {
                        $db->prepare("INSERT INTO trek_highlights (trek_id, highlight, icon, sort_order) VALUES (?, ?, ?, ?)")
                           ->execute([$id, $hl_text, $hl_icon, $hl_order++]);
                    }
                }
                ...
                for ($i = 0; $i < count($note_texts); $i++) {
                    ...
                    if (!empty($note_text)) {
                        $db->prepare("INSERT INTO trek_notes (trek_id, note, sort_order) VALUES (?, ?, ?)")
                           ->execute([$id, $note_text, $note_order++]);
                    }
                }
                ...
                for ($i = 0; $i < count($additional_note_texts); $i++) {
                    ...
                    if (!empty($add_note_text)) {
                        $db->prepare("INSERT INTO trek_additional_notes (trek_id, note, sort_order) VALUES (?, ?, ?)")
                           ->execute([$id, $add_note_text, $additional_note_order++]);
                    }
                }
                ...
                for ($i = 0; $i < count($faq_qs); $i++) {
                    ...
                    if (!empty($faq_q) && !empty($faq_a)) {
                        $db->prepare("INSERT INTO trek_faqs (trek_id, question, answer, sort_order) VALUES (?, ?, ?, ?)")
                           ->execute([$id, $faq_q, $faq_a, $faq_order++]);
                    }
                }

                set_flash_message('success', 'Trek details updated successfully!');
                header('Location: ' . SITE_URL . '/admin/treks/manage.php');
                exit();
            }
        } catch (PDOException $e) {
...
// Fetch categories for dropdown & current Trek record
try {
    $categories = $db->query("SELECT id, category_name FROM trek_categories WHERE status = 'Active'")->fetchAll();
    $gallery_cats = $db->query("SELECT id, category_name FROM gallery_categories ORDER BY category_name")->fetchAll();
...
<head>
    ...
    <title>Edit Trek Details | Admin Portal</title>
    <!-- Bootstrap 5 CSS -->
    ...
    <div id="page-content-wrapper">
        <nav class="navbar navbar-expand-lg navbar-light admin-navbar border-bottom">
            <div class="container-fluid">
                ...
                <div class="ms-auto">
                    <span class="text-muted small">Edit: <strong><?php echo htmlspecialchars($trek['title']); ?></strong></span>
                </div>
            </div>
        </nav>
        ...
        <div class="container-fluid p-4" style="max-width: 1000px;">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="fw-bold mb-0">Edit Trek Details</h3>
                ...
                <div class="admin-card">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="title" class="form-control" required value="<?php echo htmlspecialchars($trek['title']); ?>">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label d-flex justify-content-between align-items-center">
                                <span>URL Slug</span>
                            ...
                            <input type="text" name="slug" id="trek_slug" class="form-control" required value="<?php echo htmlspecialchars($trek['slug']); ?>" data-seo-counter="trek_slug_counter" data-seo-min="30" data-seo-max="50">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="category_id" class="form-select" required>
                                ...
                                <?php foreach ($categories as $cat): ?>
                                    <option value="<?php echo $cat['id']; ?>" <?php echo $trek['category_id'] == $cat['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="duration" class="form-control" required value="<?php echo htmlspecialchars($trek['duration']); ?>">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Difficulty</label>
                            <select name="difficulty" class="form-select">
                                <option value="" <?php echo empty($trek['difficulty']) ? 'selected' : ''; ?>>-- Not Specified --</option>
                                <option value="Easy" <?php echo ($trek['difficulty'] ?? '') === 'Easy' ? 'selected' : ''; ?>>Easy</option>
                                <option value="Moderate" <?php echo ($trek['difficulty'] ?? '') === 'Moderate' ? 'selected' : ''; ?>>Moderate</option>
                                <option value="Difficult" <?php echo ($trek['difficulty'] ?? '') === 'Difficult' ? 'selected' : ''; ?>>Difficult</option>
                            </select>
                        </div>

                        <div class="col-md-3">
                            <label class="form-label">Trek Distance (km) *</label>
                            <input type="number" step="0.1" name="trek_distance" class="form-control" required value="<?php echo htmlspecialchars($trek['trek_distance']); ?>">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Starting Point</label>
                            <input type="text" name="starting_point" class="form-control" placeholder="e.g. Bangalore / Trek Base" value="<?php echo htmlspecialchars($trek['starting_point'] ?? ''); ?>">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Best Season / Time</label>
                            <input type="text" name="best_season" class="form-control" placeholder="e.g. Sept - Feb / Post-Monsoon" value="<?php echo htmlspecialchars($trek['best_season'] ?? ''); ?>">
                        </div>
                        <div class="col-md-3">
                            <div class="form-check mt-4 pt-2">
                                <input type="checkbox" name="featured" class="form-check-input" id="featured_chk" <?php echo $trek['featured'] == 1 ? 'checked' : ''; ?>>
                        ...
                        <div class="col-md-6 border-end">
                            <div class="form-check mb-3">
                                <input type="checkbox" name="transport_enabled" class="form-check-input" id="transport_enabled_chk" value="1" <?php echo ($trek['transport_enabled'] ?? 0) == 1 ? 'checked' : ''; ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="with_transport_price" id="with_transport_price" class="form-control form-control-sm" value="<?php echo htmlspecialchars($trek['with_transport_price'] ?? $trek['price']); ?>">
                            </div>
                            <div class="mb-3">
                                <label class="form-label small text-muted fw-bold">Offer Price (optional)</label>
                                <input type="number" name="with_transport_offer_price" id="with_transport_offer_price" class="form-control form-control-sm" value="<?php echo htmlspecialchars($trek['with_transport_offer_price'] ?? $trek['offer_price']); ?>">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-check mb-3">
                                <input type="checkbox" name="own_transport_enabled" class="form-check-input" id="own_transport_enabled_chk" value="1" <?php echo ($trek['own_transport_enabled'] ?? 0) == 1 ? 'checked' : ''; ?>>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="without_transport_price" id="without_transport_price" class="form-control form-control-sm" value="<?php echo htmlspecialchars($trek['without_transport_price'] ?? ''); ?>">
                            </div>
                            <div class="mb-3">
                                <label class="form-label small text-muted fw-bold">Offer Price (Without Transport) (optional)</label>
                                <input type="number" name="without_transport_offer_price" id="without_transport_offer_price" class="form-control form-control-sm" value="<?php echo htmlspecialchars($trek['without_transport_offer_price'] ?? ''); ?>">
                            </div>
                        </div>
                        ...
                        <div class="col-md-12 mt-2 mb-3">
                            ...
                            <textarea name="with_transport_note" class="form-control" rows="3" placeholder="Booking notes/instructions shown when they book with transport (e.g. previous day date selector, reporting instructions)"><?php echo htmlspecialchars($trek['with_transport_note'] ?? ''); ?></textarea>
                        </div>

                        <!-- Own Transport Note -->
                        <div class="col-md-12 mt-2 mb-3">
                            <label class="form-label fw-bold small text-muted">Own Transport Booking Notes</label>
                            <textarea name="own_transport_note" class="form-control" rows="3" placeholder="Instructions shown when they book without transport (e.g. reporting time, base point instructions)"><?php echo htmlspecialchars($trek['own_transport_note'] ?? ''); ?></textarea>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <?php if (!empty($trek['image'])): ?>
                                <small class="text-success d-block mt-1">Current Image: <strong><?php echo htmlspecialchars($trek['image']); ?></strong></small>
                            <?php endif; ?>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label d-flex justify-content-between align-items-center">
                            ...
                            <input type="text" name="image_alt" id="trek_image_alt" class="form-control" placeholder="e.g. Trekkers ascending the lush misty ridge of Kudremukh mountain peak in Karnataka" value="<?php echo htmlspecialchars($trek['image_alt'] ?? ''); ?>" data-seo-counter="trek_image_alt_counter" data-seo-min="50" data-seo-max="125">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="description" class="form-control" rows="5" required><?php echo htmlspecialchars($trek['description']); ?></textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div id="itinerary_days_container">
                        <?php if (!empty($itinerary)): ?>
                            <?php foreach ($itinerary as $index => $day): 
                                $dayNum = $index + 1;
                            ?>
                                <div class="itinerary-row mb-3 p-3 border rounded bg-light" id="itinerary_day_<?php echo $dayNum; ?>">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h6 class="text-primary mb-0">Itinerary Item</h6>
                                        <button type="button" class="btn btn-sm btn-danger btn-remove-day" data-day="<?php echo $dayNum; ?>">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" name="itinerary_day[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($day['day'] ?? ('Day ' . ($index + 1))); ?>" placeholder="e.g. Day 0">
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label">Title / Heading</label>
                                            <input type="text" name="itinerary_title[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($day['title']); ?>">
                                        </div>
                                        <div class="col-md-5">
                                            <label class="form-label">Detailed Description</label>
                                            <textarea name="itinerary_desc[]" class="form-control form-control-sm" rows="2" required><?php echo htmlspecialchars($day['desc']); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <div class="admin-card">
                    ...
                    <div id="highlights_container" class="sortable-list d-flex flex-column gap-2">
                        <?php if (!empty($trek_highlights)): ?>
                            <?php foreach ($trek_highlights as $index => $hl): ?>
                                <div class="highlight-row mb-2 p-2 border rounded bg-light d-flex align-items-center gap-2">
                                    ...
                                    <select name="highlight_icons[]" class="form-select form-select-sm" style="width: auto;">
                                        <option value="fas fa-hiking" <?php echo $hl['icon'] === 'fas fa-hiking' ? 'selected' : ''; ?>>🚶 Guide</option>
                                        <option value="fas fa-fire" <?php echo $hl['icon'] === 'fas fa-fire' ? 'selected' : ''; ?>>🔥 Campfire</option>
                                        <option value="fas fa-file-contract" <?php echo $hl['icon'] === 'fas fa-file-contract' ? 'selected' : ''; ?>>📄 Permit</option>
                                        <option value="fas fa-water" <?php echo $hl['icon'] === 'fas fa-water' ? 'selected' : ''; ?>>🌊 Waterfall</option>
                                        <option value="fas fa-bus" <?php echo $hl['icon'] === 'fas fa-bus' ? 'selected' : ''; ?>>🚌 Transport</option>
                                        <option value="fas fa-utensils" <?php echo $hl['icon'] === 'fas fa-utensils' ? 'selected' : ''; ?>>🍽️ Meals</option>
                                        <option value="fas fa-campground" <?php echo $hl['icon'] === 'fas fa-campground' ? 'selected' : ''; ?>>⛺ Camping</option>
                                        <option value="fas fa-mountain" <?php echo $hl['icon'] === 'fas fa-mountain' ? 'selected' : ''; ?>>⛰️ Summit</option>
                                        <option value="fas fa-sun" <?php echo $hl['icon'] === 'fas fa-sun' ? 'selected' : ''; ?>>☀️ Sunrise</option>
                                        <option value="fas fa-camera" <?php echo $hl['icon'] === 'fas fa-camera' ? 'selected' : ''; ?>>📷 Photo</option>
                                        <option value="fas fa-check-circle" <?php echo $hl['icon'] === 'fas fa-check-circle' ? 'selected' : ''; ?>>✔️ Default</option>
                                    </select>
                                    <input type="text" name="highlights[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($hl['highlight']); ?>" placeholder="e.g. Sunrise Trek Experience">
                ...
                <div class="admin-card">
                    ...
                    <div id="notes_container" class="sortable-list d-flex flex-column gap-2">
                        <?php if (!empty($trek_notes)): ?>
                            <?php foreach ($trek_notes as $index => $note): ?>
                                <div class="note-row mb-2 p-2 border rounded bg-light d-flex align-items-center gap-2">
                                    <span class="drag-handle cursor-grab px-2 text-muted"><i class="fas fa-grip-vertical"></i></span>
                                    <input type="text" name="notes[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($note['note']); ?>" placeholder="e.g. Select Friday date for Saturday sunrise.">
                ...
                <div class="admin-card">
                    ...
                    <div id="additional_notes_container" class="sortable-list d-flex flex-column gap-2">
                        <?php if (!empty($trek_additional_notes)): ?>
                            <?php foreach ($trek_additional_notes as $index => $add_note): ?>
                                <div class="note-row mb-2 p-2 border rounded bg-light d-flex align-items-center gap-2">
                                    <span class="drag-handle cursor-grab px-2 text-muted"><i class="fas fa-grip-vertical"></i></span>
                                    <input type="text" name="additional_notes[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($add_note['note']); ?>" placeholder="e.g. Select Friday date for Saturday sunrise.">
                                    <button type="button" class="btn btn-sm btn-outline-danger btn-remove-note"><i class="fas fa-trash"></i></button>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <div id="faqs_container" class="sortable-list d-flex flex-column gap-3">
                        <?php if (!empty($trek_faqs)): ?>
                            <?php foreach ($trek_faqs as $index => $faq): ?>
                                <div class="faq-row mb-3 p-3 border rounded bg-light" id="faq_row_<?php echo $index; ?>">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-12">
                                            ...
                                            <input type="text" name="faq_question[]" class="form-control form-control-sm" required value="<?php echo htmlspecialchars($faq['question']); ?>" placeholder="e.g. What is the difficulty level?">
                                        </div>
                                        <div class="col-12">
                                            <label class="form-label small mb-1 fw-bold">Answer</label>
                                            <textarea name="faq_answer[]" class="form-control form-control-sm" rows="2" required placeholder="e.g. Easy to Moderate."><?php echo htmlspecialchars($faq['answer']); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <div class="admin-card">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea name="with_transport_inclusions" class="form-control" rows="3" placeholder="Enter line-separated items included with transport package..."><?php echo htmlspecialchars($trek['with_transport_inclusions'] ?? ''); ?></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label font-semibold">With Transport Exclusions</label>
                            <textarea name="with_transport_exclusions" class="form-control" rows="3" placeholder="Enter line-separated items excluded from transport package..."><?php echo htmlspecialchars($trek['with_transport_exclusions'] ?? ''); ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4 border-top pt-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea name="own_transport_inclusions" class="form-control" rows="3" placeholder="Enter line-separated items included with own transport package..."><?php echo htmlspecialchars($trek['own_transport_inclusions'] ?? ''); ?></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label font-semibold">Own Transport Exclusions</label>
                            <textarea name="own_transport_exclusions" class="form-control" rows="3" placeholder="Enter line-separated items excluded from own transport package..."><?php echo htmlspecialchars($trek['own_transport_exclusions'] ?? ''); ?></textarea>
                        </div>
                    </div>
                        <div class="col-md-12">
                            <label class="form-label">Things to Carry</label>
                            <textarea name="things_to_carry" class="form-control" rows="3"><?php echo htmlspecialchars($trek['things_to_carry']); ?></textarea>
                        </div>
                        <div class="col-md-12">
                            ...
                            <input type="text" name="pickup_points_txt" class="form-control" value="<?php echo htmlspecialchars($trek['pickup_points_txt'] ?? ''); ?>" placeholder="Legacy text fallback only">
                            <small class="text-muted d-block mt-1"><i class="fas fa-info-circle me-1 text-success"></i>Structured pickup points with timings, Google Maps landmarks, and routes are configured under <a href="pickups.php?trek_id=<?php echo $id; ?>" class="fw-bold text-success text-decoration-none">Manage Pickups</a>.</small>
                        </div>
                        
                        <div class="col-md-12">
                            <label class="form-label">Status</label>
                            <select name="status" class="form-select">
                                <option value="Active" <?php echo $trek['status'] === 'Active' ? 'selected' : ''; ?>>Active</option>
                                <option value="Inactive" <?php echo $trek['status'] === 'Inactive' ? 'selected' : ''; ?>>Inactive</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-card">
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            ...
                            <input type="text" name="focus_keyphrase" id="trek_focus_keyphrase" class="form-control" placeholder="e.g. Kudremukh Trek from Bangalore" value="<?php echo htmlspecialchars($trek['focus_keyphrase'] ?? ''); ?>" data-seo-counter="trek_keyphrase_counter" data-seo-min="10" data-seo-max="40">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="meta_title" id="trek_meta_title" class="form-control" placeholder="e.g. Kudremukh Trek Bangalore - 2 Day Western Ghats Trip" value="<?php echo htmlspecialchars($trek['meta_title'] ?? ''); ?>" data-seo-counter="trek_meta_title_counter" data-seo-min="50" data-seo-max="60">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea name="meta_description" id="trek_meta_description" class="form-control" rows="2" placeholder="Book the scenic Kudremukh peak trek in Chikmagalur with Karnataka Trekkers. Includes meals, certified guides, tent stays, and bus transfers from Bangalore." data-seo-counter="trek_meta_desc_counter" data-seo-min="150" data-seo-max="155"><?php echo htmlspecialchars($trek['meta_description'] ?? ''); ?></textarea>
                        </div>

                        <div class="col-12">
                            <div class="form-check form-switch p-2 border rounded bg-white">
                                <input class="form-check-input ms-0 me-2" type="checkbox" name="is_indexed" id="is_indexed" value="1" <?php echo (!isset($trek['is_indexed']) || (int)$trek['is_indexed'] === 1) ? 'checked' : ''; ?>>
                        ...
                        <div class="col-12">
                            ...
                            <div class="tag-manager-wrapper border rounded p-3 bg-light">
                                <input type="hidden" name="tags" class="tag-hidden-input" value="<?php echo htmlspecialchars($trek['tags'] ?? ''); ?>">
...
<script>
    ...
    const TREK_ID = <?php echo $id; ?>;
    const GALLERY_CATEGORIES = <?php echo json_encode($gallery_cats); ?>;
</script>
