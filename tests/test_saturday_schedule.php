...
echo "=== WEEKEND SCHEDULE AUTO-GENERATION LOGIC TEST ===\n";
...
try {
    ...
    $db->exec("INSERT INTO treks (title, slug, duration, difficulty, trek_distance, altitude, price, description, itinerary, status) 
               VALUES ('Schedule Test Trek', 'schedule-test-trek', '2 Days / 1 Night', 'Moderate', 10.0, 3000, 1500.00, 'Mock description', 'Mock itinerary', 'Active')");
    ...
    $generated_dates_1 = run_generation($trek_id, $recurring_friday, $recurring_saturday, $test_start_date, $test_until_date, $duration_days_guess);
    ...
    $generated_dates_2 = run_generation($trek_id, $recurring_friday, $recurring_saturday, $test_start_date, $test_until_date, $duration_days_guess);
    print_r($generated_dates_2);
    
    // Clean up
    $db->exec("DELETE FROM trek_dates WHERE trek_id = $trek_id");

    // Case 3: Both Friday and Saturday
    echo "\n--- Testing Case 3: Both (Friday + Saturday) ---\n";
    $recurring_friday = 1;
    $recurring_saturday = 1;
    $generated_dates_3 = run_generation($trek_id, $recurring_friday, $recurring_saturday, $test_start_date, $test_until_date, $duration_days_guess);
    print_r($generated_dates_3);
    
    // Case 4: Duplicate Protection
    echo "\n--- Testing Case 4: Duplicate Protection ---\n";
    // We run generation again with both enabled. It should return 0 new generated dates because they already exist from Case 3.
    $generated_dates_4 = run_generation($trek_id, $recurring_friday, $recurring_saturday, $test_start_date, $test_until_date, $duration_days_guess);
    echo "Generated count on second run: " . count($generated_dates_4) . " (Expected: 0)\n";
...
function run_generation($trek_id, $recurring_friday, $recurring_saturday, $start_date, $until_date, $duration_days_guess) {
    global $db;
    $generated = [];
    
    if (($recurring_friday || $recurring_saturday) && !empty($until_date)) {
        ...
        while ($current_time <= $until_time) {
            ...
            if (($recurring_friday && $day_name === 'Fri') || ($recurring_saturday && $day_name === 'Sat')) {
