 ...
 * Expected variables:
 * - $card_trek (array): The trek data row for this card (preferred to avoid global $trek collision).
 * - $trek (array): Fallback trek data row for backward compatibility.
 */
$c_trek = $card_trek ?? ($trek ?? null);

if ($c_trek):
    $difficulty_class = 'difficulty-' . strtolower($c_trek['difficulty'] ?? 'easy');
    $active_card_price = (float)get_starting_price($c_trek);
    
    // Fallback image handling and WebP detection
    $featured_img = 'assets/images/default-trek.jpg';
    if (!empty($c_trek['gallery_featured_image'])) {
        $featured_img = $c_trek['gallery_featured_image'];
    } elseif (!empty($c_trek['image'])) {
        $featured_img = $c_trek['image'];
    }
    ...
    $card_url = SITE_URL . '/treks/' . ($c_trek['slug'] ?? '');
    
    // Metadata labels
    $category_label = !empty($c_trek['category_name']) ? $c_trek['category_name'] : 'General';
    $duration_label = !empty($c_trek['duration']) ? $c_trek['duration'] : '';
    ...
    $wishlist_id = null;
    if (is_user_logged_in() && !empty($c_trek['id'])) {
        try {
            $db = Database::connect();
            $wish_stmt = $db->prepare("SELECT id FROM wishlist WHERE user_id = ? AND trek_id = ? LIMIT 1");
            $wish_stmt->execute([get_logged_in_user_id(), $c_trek['id']]);
...
<div class="trek-card-wrapper">
        ...
        <div class="trek-card">
            <div class="trek-card-image">
                <picture>
                    ...
                    <?php endif; ?>
                    <img src="<?php echo SITE_URL . '/' . ($webp_fallback ?: $featured_img); ?>" alt="<?php echo htmlspecialchars(!empty($c_trek['image_alt']) ? $c_trek['image_alt'] : ($c_trek['title'] ?? '')); ?>" width="452" height="226" loading="lazy" decoding="async">
                </picture>
                <?php if (!empty($c_trek['difficulty'])): ?>
                    <span class="trek-badge-difficulty <?php echo 'difficulty-' . strtolower($c_trek['difficulty']); ?>"><?php echo htmlspecialchars($c_trek['difficulty']); ?></span>
                <?php endif; ?>
            </div>
            <div class="trek-card-body">
                <h3 class="trek-card-title fw-bold">
                    <?php echo htmlspecialchars($c_trek['title'] ?? ''); ?>
                </h3>
    ...
    <div class="wishlist-overlay-wrapper">
        <?php if (is_user_logged_in() && !empty($c_trek['id'])): ?>
            <?php if ($is_in_wishlist): ?>
                <a href="<?php echo SITE_URL; ?>/user/wishlist.php?action=remove&trek_id=<?php echo $c_trek['id']; ?>" class="wishlist-btn active" title="Remove from Wishlist" aria-label="Remove <?php echo htmlspecialchars($c_trek['title'] ?? 'trek'); ?> from Wishlist" data-trek-id="<?php echo $c_trek['id']; ?>">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
                </a>
            <?php else: ?>
                <a href="<?php echo SITE_URL; ?>/user/wishlist.php?action=add&trek_id=<?php echo $c_trek['id']; ?>" class="wishlist-btn" title="Add to Wishlist" aria-label="Add <?php echo htmlspecialchars($c_trek['title'] ?? 'trek'); ?> to Wishlist" data-trek-id="<?php echo $c_trek['id']; ?>">
            ...
            <?php endif; ?>
        <?php else: ?>
            <a href="<?php echo SITE_URL; ?>/login.php" class="wishlist-btn" title="Login to save Trek" aria-label="Login to save <?php echo htmlspecialchars($c_trek['title'] ?? 'trek'); ?> to wishlist">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg>
            </a>
        <?php endif; ?>
    </div>
